...
// BEGIN: Import Configuration (by Lukas Heidemann)
#include <api/types.h>
#include <api/debug/console.h>
#include <api/memory/heap.h>
#include <common/memory/dlmalloc.h>
...
static void internal_malloc_stats(mstate m) {
  ...
  if (!PREACTION(m)) {
    ...
    console_print("max system bytes = ");
    console_print_hex((uintptr_t) maxfp);
    
    // fprintf(stderr, "system bytes     = %10lu\n", (unsigned long)(fp));
    console_print("\nsystem bytes = ");
    console_print_hex((uintptr_t) fp);
    
    // fprintf(stderr, "in use bytes     = %10lu\n", (unsigned long)(used));
    console_print("\nin use bytes = ");
    console_print_hex((uintptr_t) used);
    console_print("\n");
